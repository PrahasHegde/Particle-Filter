...
class ParticleFilter:
    ...
    def predict(self):
        ...
        # Add adaptive noise, with potentially different magnitudes for position and velocity
        adaptive_std = self.adaptive_noise()
        noise_pos = np.random.normal(0, adaptive_std, size=(self.n, 2))
        noise_vel = np.random.normal(0, adaptive_std * 0.5, size=(self.n, 2)) # Smaller noise for velocity

        self.particles[:, :2] += noise_pos
        self.particles[:, 2:] += noise_vel
...
class Simulator:
    ...
    def run(self):
        ...
        # Expanded ranges for better exploration, especially for velocities
        ...
        # Adjust vx and vy ranges
        # vx can range up to the max initial speed + some buffer
        vx_range = (0, max(ball.v0 for ball in self.balls) + 10) 
        # vy can become negative. Estimate a broader range based on max initial vy and potential lowest vy.
        max_initial_vy = max(ball.vy0 for ball in self.balls)
        # A simple estimation for minimum possible vy: max initial vy minus g * max_time_in_air
        # We can approximate max_time_in_air as num_steps * dt
        min_possible_vy = max_initial_vy - g * num_steps * dt 
        vy_range = (min_possible_vy - 10, max_initial_vy + 10) # Wider range for vy
        ...
        for t in range(max_timesteps):
            ...
            for ball in self.balls:
                if t < len(ball.x_obs):
                    ...
                    # If ball has landed, use its last known position as observation to keep tracking its "landed" state
    ...
    def plot(self):
        ...
        for i, ball in enumerate(self.balls):
            ...
            # This ensures RMSE is meaningful for the in-air portion.
