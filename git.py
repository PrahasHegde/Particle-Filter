...
class Simulator:
    ...
    def run(self):
        ...
        for t in range(min_timesteps):
            ...
            cost_matrix = np.linalg.norm(np.array(obs)[:, None, :] - centers[None, :, :], axis=2)
    ...
    def plot(self):
        ...
        for i, ball in enumerate(self.balls):
            plt.plot(ball.x_true, ball.y_true, color='blue', label='True Trajectory' if i == 0 else "")
            plt.scatter(ball.x_obs, ball.y_obs, s=10, alpha=0.4, color='black', label='Observations' if i == 0 else "")
            est = np.array(self.estimates_all[i])
            plt.plot(est[:, 0], est[:, 1], '--', color='red', label='Estimated Trajectory' if i == 0 else "")
